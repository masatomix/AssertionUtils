...
/**
 * 基本的には{@link TextAssertionLogic}と同様の比較を行うが、各行を、タブなどでデリミタする可変長ファイルと見なし、
 * さらに設定に記述されたカラム番号(0始まり)の列を除外して、テキスト比較する。 たとえば、日付列などがあって、そこは比較除外としたいなどを想定。
 * 
...
public class CSVAssertionLogic extends TextAssertionLogic {
    ...
    public List<String> readText(Path path, Charset enc) {
        ...
        boolean fileExport = Boolean.parseBoolean(
                AssertUtils.getSettingData("modified_file_export"));
        if (fileExport) {
            // ファイル出力。
            String newName = path.toFile().getName()
                    + AssertUtils.getModifiedExt();
    ...
    private String removeColumn(String line, int[] columns) {
        ...
        if (columns[columns.length - 1] >= originalStrs.length) {
            log.debug("このレコードはカラム数が、除外列番号より少ないです。{}", line);
            return line;
        }
